...
//   spin   "0" to hold still (default: slow oscillation)
...
function stampTexture(label, sub, band, accent) {
  ...
  g.fillStyle = band; g.fillRect(0, 0, c.width, c.height);
  g.globalAlpha = 0.05;
...
function wrappedBar(opts) {
  const { tone, label, sub, band, accent, length = 0.088, depth = 0.062, height = 0.03 } = opts;
  const group = new THREE.Group();
  const soapMat = new THREE.MeshStandardMaterial({ color: tone, roughness: 0.62, metalness: 0 });
  soapMat.name = 'soap';
  const soap = new THREE.Mesh(roundedPlateGeometry(length, depth, height, 0.009), soapMat);
  soap.name = 'bar';
  soap.castShadow = true; soap.receiveShadow = true;
  group.add(soap);

  const kraftMat = new THREE.MeshStandardMaterial({ color: band, roughness: 0.92, metalness: 0 });
  kraftMat.name = 'kraft';
  const stampMat = new THREE.MeshStandardMaterial({ map: stampTexture(label, sub, band, accent), roughness: 0.9 });
  ...
  const bandMesh = new THREE.Mesh(bandGeo, [kraftMat, kraftMat, stampMat, kraftMat, kraftMat, kraftMat]);
...
function drawerBox(opts) {
  ...
  const W = 0.20, D = 0.11, H = 0.05, T = 0.004;
  const sleeve = new THREE.Group();
  const top = new THREE.Mesh(new THREE.BoxGeometry(W, T, D), kraft);
  top.name = 'sleeve-top'; top.position.y = H - T / 2;
  const bottom = new THREE.Mesh(new THREE.BoxGeometry(W, T, D), kraft);
  bottom.name = 'sleeve-bottom'; bottom.position.y = T / 2;
  const left = new THREE.Mesh(new THREE.BoxGeometry(T, H, D), kraft);
  left.name = 'sleeve-left'; left.position.set(-W / 2 + T / 2, H / 2, 0);
  const right = left.clone(); right.name = 'sleeve-right'; right.position.x = W / 2 - T / 2;
  const back = new THREE.Mesh(new THREE.BoxGeometry(W, H, T), kraft);
  back.name = 'sleeve-back'; back.position.set(0, H / 2, -D / 2 + T / 2);
  [top, bottom, left, right, back].forEach(m => { m.castShadow = true; m.receiveShadow = true; sleeve.add(m); });
  const markMat = new THREE.MeshStandardMaterial({ map: stampTexture('Soap Notes', 'the complete trio', band, accent), roughness: 0.9 });
  ...
  tray.name = 'tray';
  const tW = W - T * 2.6, tD = D - T * 2, tH = 0.026;
  const floor = new THREE.Mesh(new THREE.BoxGeometry(tW, T, tD), lining);
  floor.name = 'tray-floor'; floor.position.y = T * 1.2;
  const wallF = new THREE.Mesh(new THREE.BoxGeometry(tW, tH, T), kraft);
  wallF.name = 'tray-front'; wallF.position.set(0, tH / 2 + T, tD / 2 - T / 2);
  const wallB = wallF.clone(); wallB.name = 'tray-back'; wallB.position.z = -tD / 2 + T / 2;
  const wallL = new THREE.Mesh(new THREE.BoxGeometry(T, tH, tD), kraft);
  wallL.name = 'tray-left'; wallL.position.set(-tW / 2 + T / 2, tH / 2 + T, 0);
  const wallR = wallL.clone(); wallR.name = 'tray-right'; wallR.position.x = tW / 2 - T / 2;
  [floor, wallF, wallB, wallL, wallR].forEach(m => { m.castShadow = true; m.receiveShadow = true; tray.add(m); });

  const tones = ['#EFE6D2', '#E9D2CC', '#EFE2C6'];
  const names = [['Eros', 'no. 01'], ['Kairos', 'no. 02'], ['Logos', 'no. 03']];
  tones.forEach((tone, i) => {
    const bar = wrappedBar({ tone, label: names[i][0], sub: names[i][1], band, accent, length: 0.056, depth: 0.052, height: 0.022 });
...
class SoapBar3D extends HTMLElement {
  ...
  static get observedAttributes() { return ['accent', 'band', 'tone', 'label', 'sub']; }
  ...
  _build() {
    ...
    const shadowPlane = new THREE.Mesh(new THREE.PlaneGeometry(1, 1), new THREE.ShadowMaterial({ opacity: 0.22 }));
    ...
    this._spin = this.getAttribute('spin') !== '0';
  ...
  _populate() {
    ...
    if (mode === 'trio') {
      const tones = ['#EFE6D2', '#E9D2CC', '#EFE2C6'];
      const labels = [['Agora', 'the origin bar'], ['Eros', 'no. 01'], ['Kairos', 'no. 02']];
      tones.forEach((t, i) => {
      ...
      this._radius = 0.36; this._lift = 0.014;
    } else if (mode === 'box') {
      this.rig.add(drawerBox({ band, accent }));
      this._radius = 0.42; this._lift = 0.035;
    } else {
      this.rig.add(wrappedBar({
        tone,
        label: this.getAttribute('label') || 'Soap Notes',
        sub: this.getAttribute('sub') || '',
        band, accent
      }));
      this._radius = 0.2; this._lift = 0.014;
    }
  }
  ...
  _frame(t) {
    ...
    if (gl && gl.isContextLost && gl.isContextLost()) { shared = null; return; }
